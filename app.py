...
def detectar_ganadores(secuencia_cartones, secuencia_llamados, modos):
    """
    Simula la secuencia de bolas y registra ganadores por cada forma de ganar activa.
    Puede haber varios ganadores (distintos cartones y/o distintos modos).
    """
    estados = []
...
def formatear_reporte_ganadores(secuencia_llamados, modos, ganadores):
    """Genera el bloque de texto con ganadores para incluir al final del TXT."""
    lineas = [
        "",
        "========== GANADORES ==========",
        "Secuencia utilizada: " + ", ".join(str(n) for n in secuencia_llamados),
        "",
    ]

    nombres_modos = [m["nombre"] for m in modos]
    modos_con_ganador = set()

    if ganadores:
        for g in ganadores:
            lineas.append(
                "{nombre} ganador cartón {carton} (bola #{bola}: {num})".format(
                    nombre=g["nombre_modo"],
                    carton=g["numero_carton"],
                    bola=g["bola"],
                    num=g["numero_cantado"],
                )
            )
            modos_con_ganador.add(g["nombre_modo"])
    else:
        lineas.append("No hubo ganadores con esta secuencia de números.")

    sin_ganador = [nombre for nombre in nombres_modos if nombre not in modos_con_ganador]
    if sin_ganador and ganadores:
        lineas.append("")
        lineas.append("Formas de ganar sin ganador en esta tanda:")
        for nombre in sin_ganador:
